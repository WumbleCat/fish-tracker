...
The deployment target is **decided (2026-08-23): Vercel**, serverless Python via `backend/api/index.py` with `vercel.json` rewriting every path into the ASGI app and `requirements.txt` as the deploy manifest (hand-kept in step with `pyproject.toml`). The `fish-tracker` Vercel project is git-linked to the repo root, so the root also carries a thin shim (`/vercel.json`, `/api/index.py`, `/requirements.txt`, added 2026-08-24) that builds the same backend — a git push to main auto-deploys the API rather than clobbering it. The web app is its own Vercel project (`fish-tracker-app`, root directory `web/`, since 2026-08-24) serving https://fish-tracker-app.vercel.app — also git-deployed on push to main, with `web/vercel.json` carrying the SPA rewrite and build config coming from the committed `web/.env.production`. These two are the only Vercel projects; earlier file-upload and misrooted duplicates were deleted. On Vercel the engine uses NullPool and disables psycopg prepared statements (PgBouncer). Env vars live in the Vercel dashboard. Local development still runs against the local stack:

```
uv run uvicorn app.main:app --reload --port 8000
```

Environment (never committed — see `repo` for the `.env` contract). On Vercel, the Supabase↔Vercel integration's injected vars are honoured as fallbacks (added 2026-08-24): `POSTGRES_URL`/`POSTGRES_URL_NON_POOLING` → `DATABASE_URL` (normalized to the psycopg driver, integration query params stripped, sslmode kept, raw un-percent-encoded credentials re-encoded — the integration injects the database password raw, and one with `@`/`?`/spaces otherwise breaks or silently corrupts URL parsing), `SUPABASE_SECRET_KEY` → service key. The integration does NOT inject the legacy `SUPABASE_JWT_SECRET`; until it is set by hand, production refuses ALL HS256 tokens and guest joins return 503 `guest_unavailable` — running HS256 on the public dev secret would make every guest token forgeable. `GET /api/health` reports `db: ok|unavailable` from a live `SELECT 1` (added 2026-08-24) — check it before log-diving a 500:
