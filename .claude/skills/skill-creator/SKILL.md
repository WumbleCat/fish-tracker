---
name: skill-creator
description: Create or update a Claude skill with appropriately scoped instructions and any needed supporting resources. Use when the user asks to "create a skill", "write a SKILL.md", "add a skill to this project", or wants an existing skill's description, structure, or bundled resources improved.
license: Apache-2.0
---

# Skill Creator

Create skills that give Claude useful, non-obvious guidance without constraining unrelated work.

## Core Principles

**Assume Claude is already capable.** Include only information that changes its decisions or improves its work. Remove generic advice, repeated instructions, speculative edge cases, and examples that do not materially clarify the task.
...
**Keep discovery cheap and precise.** Skill names and descriptions sit in context before a skill is loaded. Describe the actual capability and when it applies, including concrete phrases a user would say, and add exclusions only when they prevent likely misrouting. Avoid exhaustive capability lists and catchalls that attract unrelated requests.
...
Claude Code discovers skills by scanning for `SKILL.md` one level deep in:

- `~/.claude/skills/<skill-name>/` — personal skills, available in every project.
- `<project>/.claude/skills/<skill-name>/` — project skills, shared with everyone who checks out the repository.
- `<plugin>/skills/<skill-name>/` — skills distributed as part of a plugin.

Name the folder after the skill; the folder name and the frontmatter `name` should match.

### SKILL.md

The YAML frontmatter identifies the skill and determines when it should be considered. Include the required `name` and `description`; add optional fields only when they change behavior the user asked for. See [references/frontmatter.md](references/frontmatter.md) for the supported fields and their effects.
...
1. **Name and description:** Always in context during skill selection, so keep them concise and discriminating.
2. **SKILL.md body:** Loaded when the skill applies, so keep its instructions relevant to that task.
3. **Supporting resources:** Read or executed only when the current task actually needs them.
...
- **Validation:** Run new or changed scripts to verify their behavior. Scripts can usually be executed without loading their full implementation into context, although Claude may need to read them when patching or adapting them.
...
Keep references focused on maintained, task-specific information that changes Claude's decisions. Avoid copied manuals, exhaustive catalogs, and generic tutorials already available from authoritative sources. Before removing existing resources, inspect their callers and purpose.
...
A skill is reachable two ways at once, both driven by the same frontmatter:

- Claude invokes it automatically through the Skill tool when the description matches the request.
- The user invokes it explicitly by typing `/skill-name`.

Automatic invocation is the default. Set `disable-model-invocation: true` only when the user explicitly wants a skill that runs on request alone. Do not infer that setting from sensitive operations or required approvals: keep the skill discoverable and require authorization immediately before the actual mutation. Preserve an existing skill's invocation settings unless the user asks to change them.

Restrict tools with `allowed-tools` only when the skill's workflow genuinely should not reach further — an over-narrow list silently breaks the skill in unrelated tasks.
...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise put a skill that belongs to the current codebase in `.claude/skills/`, and a skill the user wants everywhere in `~/.claude/skills/`. When that choice matters and stays unclear, ask which one.
...
scripts/init_skill.py my-skill --path ~/.claude/skills --resources references
```
...
The initializer creates the skill directory and a concise `SKILL.md` starter. It creates resource directories and example files only when requested.

### Write the Instructions

The frontmatter `description` should briefly explain what the skill does and when it applies, in the third person, using phrases a user would actually say. Include a meaningful boundary when similar requests should not activate the skill.
...
Write only the instructions needed for another Claude instance to perform the task well. State the desired outcome, non-obvious context, real constraints, and relevant references or tools. Prefer imperative, verb-first phrasing over second person. Preserve the user's explicit choices and existing authorization boundaries. Avoid prescribing a fixed structure, process, or number of steps when the task does not require one.
...
Confirm the skill is actually picked up: a new session lists it among the available skills, and `/skill-name` invokes it.
...
```text
Use the skill at /path/to/skill-name to complete this realistic request.
```
