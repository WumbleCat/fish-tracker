...
The front door is the **"Deal me in" direction (decided 2026-08-24, from the Claude Design first-run project)**: always-dark felt (the `felt-*` palette, in both themes — it is the identity, not a surface), "Sit down" with six round chip tiles for the code (`components/CodeTiles`: one hidden input behind the tiles, so typing, backspace and paste all work; A–Z 0–9 only, folded to uppercase), a pill name field, and a pill "Deal me in" button that lights mint when six characters and a name are in. Sign-in is the card on the right ("HOST OR REGULAR"), with sign-up and magic link as small links. A **join link** `/join/<CODE>` (`routes/JoinLink`) opens this door with the tiles filled — or seats a signed-in player directly. The host's **first game** is created inline on the empty Sessions page ("HOST, FIRST GAME": name, currency, seats → *Open the table*), not in a modal; the session then opens on a **dealt table** empty state ("Nobody's bought in yet" / *Log a buy-in* / *Copy link*). The session header shows `n/9 seats` and *Copy link* / *Copy code* chips while the game is joinable; a full table's refusal reads "That table is full — nine seats, all taken."

- **Sign in / sign up** — email + password and magic link, via Supabase Auth. One card, toggling between modes, not two routes that lose typed input when you switch.
- **Join with a code** — reachable without an account. Entering a code and a display name gets you into the game as a guest. This path is the whole left side of the landing screen: the person using it is standing in someone's kitchen with the code read out to them.
- **Guests are remembered.** The guest session lives in localStorage; `lib/auth` refreshes the token on every visit and whenever the tab becomes visible again, and drops the session only when the server says the token is invalid — never on a network blip or a closed game.
