 ...
 * the exponent comes from the game, never a hard-coded 2. Mirrors
 * web/src/lib/money.ts on purpose — no shared package until a third
 * consumer exists.
 */
...
/** "12.34" — digits only, exponent decimals exactly, integer arithmetic. */
...
/** "+£40" style for pending chips; zero renders unsigned. */
...
/**
 * Parse keypad input to minor units with integer arithmetic — never through
 * a float. Rejects more decimals than the exponent allows, negatives, and
